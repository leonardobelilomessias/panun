import imageexample from '@/app/public/images/Home/searchImage.png'
...
export function CardHouse({area,bathrooms,bedrooms,description,garage,price,title,neighborhood, city, propurse,id}:CardHouseProps) {
    return (
        <Link href={`/imovel/${title.split(" ").join('-')}/${id}`} className="min-w-72 max-w-[24rem] h-[28rem]  md:h-[28rem] md:pb-4 flex flex-col bg-white  border-primaryPalet  rounded-lg   shadow-primaryPalet drop-shadow-xl">
            <div className='object-cover relative'>
                <p className='bg-primaryPalet p-2 rounded-xl text-white absolute top-2 right-2 text-xs font-bold px-4'>{propurse}</p>
                <Image 
                    src={imageexample} 
                    width={300} 
                    height={300} 
