...
import { deleteCommentQuestionById } from "@/lib/supabase/queries/server/questions/deleteCommentQuestionById";
...
export function DeleteCommentButton({ commentId, userId, currentUserId }: DeleteCommentButtonProps) {
  ...
  const handleDelete = async () => {
    try {
      const result = await deleteCommentQuestionById(commentId);
      if (result.success) {
        toast({
          title: "Sucesso",
