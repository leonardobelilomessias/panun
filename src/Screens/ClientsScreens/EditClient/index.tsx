...
import { ArrowLeft, Building2, CalendarIcon, Home, Mail, MapPin, Phone, Save, UserIcon } from "lucide-react"
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from "@/components/ui/card"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { Badge } from "@/components/ui/badge"
import { Skeleton } from "@/components/ui/skeleton"
import { getClientById } from "@/lib/supabase/queries/client/Clients/getClientById"
import { updateClientById } from "@/lib/supabase/queries/client/Clients/updateClientById"
...
export function EditClientScreen({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function fetchClientData() {
      ...
      try {
        ...
        // Atualiza os valores do formulário quando os dados do cliente são carregados
        if (data) {
  ...
  async function onSubmit(data: z.infer<typeof clientSchema>) {
    setIsSaving(true)
    try {
      const resp = await updateClientById(id, data)

      if (resp.error) {
        toast({
          title: "Erro!",
          description: `Erro ao atualizar cliente: ${resp.error.message}`,
          variant: "destructive",
        })
        return
      }
