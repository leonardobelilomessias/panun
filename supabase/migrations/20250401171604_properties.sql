...
CREATE POLICY "Herda permissões da propriedade"
    ...
    USING (EXISTS (
        ...
        WHERE id = property_id AND (
            ...
            responsible_agent IN (SELECT id FROM public.agents WHERE user_id = auth.uid())
        )
    ));
...
CREATE POLICY "Agentes podem ver leads de suas propriedades"
    ...
    USING (property_id IN (
        ...
        WHERE responsible_agent IN (SELECT id FROM public.agents WHERE user_id = auth.uid())
    ));
